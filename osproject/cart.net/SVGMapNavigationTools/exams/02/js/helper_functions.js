...
Copyright (C) <2000-2007>  <Andreas Neumann>
...
/* see documentation at http://www.carto.net/papers/svg/networkrequests/ */
...
function getData(url,callBackFunction,returnFormat,method,postText,additionalParams) {
	this.url = url; //the url of the resource
	this.callBackFunction = callBackFunction; //the callback function or resource
	this.returnFormat = returnFormat; //either "xml" or "json"
	this.method = method; //either "get" or "post"
	this.additionalParams = additionalParams;
	if (method != "get" && method != "post") {
		alert("Error in network request: parameter 'method' must be 'get' or 'post'");
	}
	this.postText = postText; //holding the text information to be posted to the server (string or undefined)
	this.xmlRequest = null; //reference to the XMLHttpRequest
} 
...
//this is the callback method for the getURL() or postURL() function
