...
{
	class MapRegExp {
		public var constA:String = null;
		public var source:String = null;
		public var global:Boolean = false;
		public var ignoreCase:Boolean = false;
		public var multiline:Boolean = false;
		public var lastIndex:Number = 0;
		public static var _xrStatic:Number = 0;
		public var _xr:Number = 0;
		public static var _xp:Number = 0;
		public static var _xxa:Array = null;
		public static var _xxlp:Number = 0;
		public var _xq:Number = 0;
		public var _xqc:Number = 0;
		public static var d:Number = 0;
		public static var _xiStatic:Number = 0;
		...
		private static var _setString:Boolean = MapRegExp.setStringMethods();
	
		function MapRegExp(...arguments) {
			if (arguments[0] == null) {
			} else {
				constA = "RegExp";
		...
		public function compile() {
			...
			if (arguments.length < 3) {
				root = true;
			MapRegExp._xrStatic = 1;
				//Paren counter
				i = 0;
			} else {
				root = false;
				this._xr = MapRegExp._xrStatic++;
				i = arguments[2];
			}
			this.lastIndex = 0;
			/*
			...
			for (i=i; i<l; ++i) {
				...
				if (char) {
					...
					if (thischar == "(") {
						//Opening paren
						ex = new MapRegExp(re, (this.ignoreCase ? "gi" : "g"), i+1);
						i = MapRegExp._xiStatic;
						q[qc].t = 3;
						thischar = ex;
						...
						while (i<l && (s=re.substring( 1+i++, 1)) != "]") {
							if (range) {
								...
								cb = s == "\\" ? (s == "b" ? String.fromCharCode(8) : re.substring( 1+i++, 1)) : s;
								var str:String; 
								str = thischar.substring( thischar.length, 1);
								ca = str.charCodeAt(0)+1;
								while (cb>=(s=String.fromCharCode(ca++))) {
									thischar += s;
								}
								range = false;
							} else {
								if (s == "-" && thischar.length >0) {
									...
									if (s == "\\") {
										...
										if (s == "d") {
											...
											thischar += String.fromCharCode(8);
										} else if (s == "\\") {
											thischar += s;
										}
									} else {
		...
		public function test(...arguments) {
					...
					if (q[i].t == 7) {
						...
						if (q[i].c == -10) {
							...
							if (q[i].t == 0) {
								...
								if (re._xr>MapRegExp._xxlp) {
								MapRegExp._xxlp = re._xr;
								}
							MapRegExp._xxa[Number(re._xr)] = MapRegExp._xxlm;
							} else if (q[i].t == 4) {
								//Back reference to paren
								if (MapRegExp._xp>=(c=Number(q[i].s))) {
									c = MapRegExp._xxa[c];
			...
			if (r.i<0) {
				...
				ix = r.li;
				this._xi = r.i;
			MapRegExp._xxlm = str_.substring( r.i+1, ix-r.i);
			MapRegExp._xxlc = str_.substring( 1, r.i);
			MapRegExp._xxrc = str_.substring( ix+1, l-ix);
				if (ix == r.i) {
					ix++;
				}
				this.lastIndex = ix;
				if (MapRegExp._xp-- == 1) {
				MapRegExp.lastMatch = MapRegExp._xxlm;
				MapRegExp.leftContext = MapRegExp._xxlc;
				MapRegExp.rightContext = MapRegExp._xxrc;
				MapRegExp._xaStatic = MapRegExp._xxa;
				MapRegExp.lastParen = MapRegExp._xxa[Number(MapRegExp._xxlp)];
					for (i=1; i<10; i++) {
					MapRegExp["$"+i] = MapRegExp._xaStatic[Number(i)];
					}
				}
				return true;
			}
		...
		public function exec() {
			...
			if (t) {
				...
				ra.index = this._xi;
				ra.input = str;
				ra[0] = MapRegExp.lastMatch;
				var l = MapRegExp._xaStatic.length;
				for (var i = 1; i<l; i++) {
					ra[i] = MapRegExp._xaStatic[Number(i)];
				}
			} else {
				ra = null;
			}
		...
		public static function setStringMethods() {
			...
			String.prototype.replace = function() {
				...
				if (re.global) {
					...
					while (re.test(s)) {
						...
						while (i<l) {
							...
							if (c == "$" && pc != "\\") {
								...
								if (isNaN(Number(c)) || Number(c)>9) {
									...
									nrs += MapRegExp._xaStatic[Number(c)];
								}
							} else {
								nrs += c;
							}
						...
						ix = re._xi+(MapRegExp.lastMatch).length;
						ip = re.lastIndex;
					}
					re.lastIndex = ip;
				} else {
					if (re.test(s)) {
						r += MapRegExp.leftContext+rs;
					}
				}
				r += re.lastIndex == 0 ? s : MapRegExp.rightContext;
				return r;
			};
			...
			String.prototype.split = function() {
				if (typeof (arguments[0]) == "object" && arguments[0].constA == "RegExp") {
					var re = arguments[0];
	...
	//trace(re._xi + " " + ix + " " + MapRegExp.lastMatch);
						if (re._xi != ix) {
					...
					if (rc == 0) {
						...
						if (rc<lm && (MapRegExp.rightContext).length>0) {
							ra[rc++] = MapRegExp.rightContext;
						}
					}
					re.global = gs;
					return ra;
