package info.osmatrix.map{
	import info.osmatrix.map.events.MapEvent;
	...
	public class MapMovieClip extends MovieClip {
		...
		public function set zoomInSelected(val:Boolean):void {
			...
			if (val) {
				...
				addEventListener(MouseEvent.CLICK, zoomDoubleClickMap);
				dispatchEvent(new MapEvent(MapEvent.MAP_PROPERTY_ON));
			} else {
				removeEventListener(MouseEvent.CLICK, zoomDoubleClickMap);
		...
		public function zoomMap(zoomValue:Number, regPoint:Point):void {
			...
			var globOrgPoint:Point = new Point(x, y);
